...
import { FiArrowLeft, FiPlus, FiTrash2, FiCheck, FiClock, FiAlertTriangle } from 'react-icons/fi';
...
function MeetingDetail() {
  ...
  return (
    <div>
      <Link to="/meetings" className="btn btn-secondary mb-24"><FiArrowLeft /> Back to Meetings</Link>

      <div className="detail-header">
        <h2>{meeting.title}</h2>
        <div className="detail-meta">
          <span>Date: {new Date(meeting.date).toLocaleDateString()}</span>
          {meeting.attendees?.length > 0 && <span>Attendees: {meeting.attendees.join(', ')}</span>}
        </div>
      </div>
              ...
              <table>
                ...
                <tbody>
                  {meeting.decisions.map(d => (
                    <tr key={d._id}>
                      <td><Link to={`/decisions/${d._id}`}>{d.title}</Link></td>
                      <td><span className={`badge ${statusBadge(d.outcome)}`}>{d.outcome}</span></td>
                  ...
                  {meeting.actions.map(a => (
                    <tr key={a._id}>
                      ...
                      <td className="text-sm">
                        {a.deadline ? (
                          <span style={{ color: isOverdue(a.deadline, a.status) ? 'var(--danger)' : 'inherit' }}>
                            {isOverdue(a.deadline, a.status) && <FiAlertTriangle style={{ marginRight: 4 }} />}
                      ...
                      <td>
                        <div className="inline-actions">
                          {a.status !== 'completed' && (
                            <button className="btn btn-success btn-sm" onClick={() => updateActionStatus(a._id, 'completed')} title="Mark complete">
                              <FiCheck />
                            </button>
                          )}
                          {a.status === 'pending' && (
                            <button className="btn btn-secondary btn-sm" onClick={() => updateActionStatus(a._id, 'in_progress')} title="Start">
                              <FiClock />
                            </button>
                          )}
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={addAction}>
              <div className="form-group">
                ...
                <input className="form-control" required value={actionForm.title}
                  onChange={e => setActionForm({ ...actionForm, title: e.target.value })} />
              </div>
              <div className="form-group">
                <label>Owner *</label>
                <input className="form-control" required value={actionForm.owner}
                  onChange={e => setActionForm({ ...actionForm, owner: e.target.value })} />
              </div>
              <div className="form-group">
                <label>Deadline</label>
                <input className="form-control" type="date" value={actionForm.deadline}
